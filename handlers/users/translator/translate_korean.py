...
@dp.callback_query_handler(text='back', state='translate_korean')
...
@rate_limit(limit=5)
@dp.callback_query_handler(text='korean_lang')
async def ready_translation(call: types.CallbackQuery, state: FSMContext):
    await call.answer(cache_time=5)
    global msg_text_korean
    msg_text_korean = await call.message.edit_text(f'Пожалуйста, введите слово или текст для перевода: ', reply_markup=kb_back)
    await state.set_state("translate_korean")


@dp.message_handler(state='translate_korean')  # ---- получаем стейт translates (который назаначен выше)
async def get_translate(message: types.Message, state: FSMContext):
    await msg_text_korean.delete()
    text = message.text
    result = translator.translate(f'{text}', dest='ko').text
