...
@dp.callback_query_handler(text='back', state='translates')
...
@rate_limit(limit=5)
@dp.callback_query_handler(text='eng_lang')
async def ready_translation(call: types.CallbackQuery, state: FSMContext):
    await call.answer(cache_time=5)
    global msg_text_eng
    msg_text_eng = await call.message.edit_text(f'Пожалуйста, введите слово или текст для перевода: ', reply_markup=kb_back)
    await state.set_state("translates")


@dp.message_handler(state='translates')  # ---- получаем стейт translates (который назаначен выше)
async def get_translate(message: types.Message, state: FSMContext):
    await msg_text_eng.delete()
    text = message.text
    result = translator.translate(f'{text}', dest='en').text
