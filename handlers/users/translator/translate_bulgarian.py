...
@dp.callback_query_handler(text='back', state='translate_bulgarian')
...
@rate_limit(limit=5)
@dp.callback_query_handler(text='bulgarian_lang')
async def ready_translation(call: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state("translate_bulgarian")


@dp.message_handler(state='translate_bulgarian')  # ---- получаем стейт translates (который назаначен выше)
async def get_translate(message: types.Message, state: FSMContext):
    await msg_text_bulgarian.delete()
    text = message.text
    result = translator.translate(f'{text}', dest='bg').text
